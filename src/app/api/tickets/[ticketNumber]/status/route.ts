...
export async function POST(
    ...
    try {
        ...
        const canUpdate = await hasPermission(user, 'tickets', 'update') ||
            existingTicket.assignedToId === userId ||
            existingTicket.reportedById === userId; // Allow reporter to cancel their own ticket
        ...
        const ticketWithInspection = await prisma.ticket.findUnique({
            where: { ticketNumber },
            select: { inspectionId: true, id: true },
        });
        ...
        if (ticketWithInspection?.inspectionId) {
            try {
                ...
                await syncTicketStatusToInspection(ticketWithInspection.id);
                ...
                if (body.status === 'resolved' || body.status === 'closed') {
                    ...
                    await sendTemplatedEmail({
                        ...
                        variables: {
                            ...
                            ticketUrl: `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/tickets/${ticket.ticketNumber}`,
                        },
                        overrideRecipients: {
