...
import { AlertCircle, CheckCircle, Smartphone, Calendar, User, ClipboardCheck, History, Package, MapPin, DollarSign, XCircle } from 'lucide-react';
...
export default function ScanPage({ params }: ScanPageProps) {
    ...
    return (
        <div className="w-full min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 pb-8">
            {/* Header */}
            <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white p-6 pb-12 rounded-b-3xl shadow-xl">
                                ...
                                {asset.borrowHistory.map((item: any, idx: number) => (
                                    <div key={idx} className="p-3 bg-slate-50 rounded-lg">
                                        <div className="flex items-center justify-between mb-2">
                                            ...
                                            <span className={`text-xs px-2 py-1 rounded-full font-medium ${item.status === 'Returned' ? 'bg-gray-100 text-gray-700' : 'bg-blue-100 text-blue-700'
                                                }`}>
                                                {item.status}
                                            </span>
                                        </div>
                                ...
                                {asset.inspections.map((inspection: any) => (
                                    <div key={inspection.id} className="p-3 bg-slate-50 rounded-lg">
                                        <div className="flex items-center justify-between mb-2">
                                            ...
                                            <span className={`text-xs px-2 py-1 rounded-full font-medium ${inspection.inspectionType === 'checkout' ? 'bg-blue-100 text-blue-700' :
